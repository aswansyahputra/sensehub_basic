metodeRata2UI <- function(id) {
  ns <- NS(id)
  tagList(
    fluidRow(
      column(
        4,
        h1("Rate-all-that-Apply II"),
        wellPanel(
          ...
          conditionalPanel(
            ...
            ns = ns,
            helpText(tags$small("Contoh menggunakan data RATA dari panelis konsumen. Pengaturan parameter adalah sebagai berikut: Panelis = 'panelist', Sampel = 'sample', Atribut Utama= 'intense colour' - 'light taste', dan tidak ada Atribut Tambahan."))
          ),
          conditionalPanel(
...
metodeRata2 <- function(input, output, session) {
  ns <- session$ns
  ...
  mentah <- eventReactive(input$unggah, {
    if (input$sumber_data == "contoh") {
      read_csv("data/iced_coffee_rata.csv")
  ...
  lokal_atributUtama <- eventReactive(input$terapkan, {
    ...
    mentah() %>%
      ...
      mutate(Nilai = replace(Nilai, Nilai >= 1, 1)) %>% 
  ...
  statistik_atributUtama <- reactive({
    ...
    lokal_atributUtama() %>%
      ...
      mutate(Q_stat = round(Q_stat, 3), 
             ...
             Keterangan = if_else(P_value < 0.01, "**",
  ...
  global <- eventReactive(input$terapkan, {
    ...
    if (is.null(input$atribut_tambahan)) {
      df <- mentah() %>%
        ...
        mutate_if(is.numeric, .funs = funs(replace(., . >= 1, 1))) %>% 
